use super::{attacks, Bitboard, Color, Move, MoveKind, Piece, PieceType, ThreeCheckState};
...
fn pop_least_valuable(state: &ThreeCheckState, occupancy: &mut Bitboard, attackers: Bitboard) -> Option<PieceType> {
	for i in 0..6 {
...
pub fn see(state: &ThreeCheckState, mv: Move, threshold: i32) -> bool {
	...
	loop {
		...
		if next == Some(PieceType::Pawn) || next == Some(PieceType::Bishop) || next == Some(PieceType::Queen) {
			attackers |= attacks::bishop_attacks(square, occupancy) & (state.pieces(PieceType::Bishop) | state.pieces(PieceType::Queen));
		}

		if next == Some(PieceType::Rook) || next == Some(PieceType::Queen) {
			attackers |= attacks::rook_attacks(square, occupancy) & (state.pieces(PieceType::Rook) | state.pieces(PieceType::Queen));
		}

		attackers &= occupancy;
		score = -score - 1 - if let Some(pt) = next {
			see_piece_value(pt)
		} else {
			0
		};
